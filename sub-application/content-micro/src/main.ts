...
import { mountContentMicro, unmountContentMicro } from './bootstrap'
...
renderWithQiankun({
  ...
  async mount(props) {
    ...
    const el = (c?.querySelector('#app') ?? c ?? document.querySelector('#app')) as
      ...
      | null
    if (!el) {
      console.error('[content-micro] mount target not found')
      return
    }
    await mountContentMicro(el)
  },
  unmount() {
    unmountContentMicro()
    return Promise.resolve()
...
if (!qiankunWindow.__POWERED_BY_QIANKUN__) {
  ...
  if (!allowStandalone) {
    ...
    if (el) void mountContentMicro(el)
  }
}
